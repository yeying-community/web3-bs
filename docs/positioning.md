...
当前库的定位**总体合理**，但建议把表述从“注入钱包 SDK”收紧为：

> **浏览器端 DApp 认证、授权与存储接入 SDK**

更具体一点：

> **以 EIP-1193 钱包为优先入口，统一封装 Provider 发现、SIWE 登录、UCAN 授权、中心化 UCAN、WebDAV 存储接入的浏览器端 SDK**

这一定义比“钱包插件中间库”更准确，因为当前代码已经明显超出“钱包连接”本身。
...
这说明它的实际角色不是“只帮你找钱包”，而是“帮 DApp 在浏览器里完成认证和资源访问接入”。
...
- 一个**浏览器端** SDK
- 一个**前端集成层** SDK
- 一个以**钱包优先**为主，但也支持**中心化认证补充路径**的 SDK
- 一个面向 **DApp -> 后端服务 / WebDAV 存储** 的访问编排 SDK
...
- `signMessage` / `loginWithChallenge` 更接近通用 EIP-1193 能力
...
> `@yeying-community/web3-bs` 是一个浏览器端 DApp 接入 SDK，用于统一封装 EIP-1193 Provider 发现、SIWE 登录、UCAN 多后端授权、中心化 UCAN 和 WebDAV 存储接入能力。它面向前端 DApp 集成，不负责链上交易构造，也不承担服务端鉴权逻辑。
...
2. 后端登录：challenge 登录、token 管理、鉴权请求
3. 多后端授权：UCAN session / root / invocation
4. 存储接入：WebDAV client、应用目录与 prefix
5. 无插件移动端：中心化 UCAN / SSO / WebAuthn 方案
...
其中“钱包”是第一入口，“UCAN / JWT / WebDAV / 中心化移动端方案”是围绕 DApp 接后端服务的能力组合。
